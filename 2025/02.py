...
def check_very_silly_single_threaded(all_numbers):
    ...
    for idx, n in enumerate(all_numbers):
        ...
        if (idx + 1) % max(1, total // 100) == 0:
            ...
            sys.stdout.write(
                f"\rChecking is_very_silly: {idx+1}/{total} ({percent:5.1f}%)"
            )
            sys.stdout.flush()
    sys.stdout.write("\n")
    return results
...
def check_very_silly_multi_threaded(all_numbers):
    ...
    def update_progress():
        while True:
            ...
            sys.stdout.write(
                f"\rChecking is_very_silly: {done}/{total_numbers} ({percent:5.1f}%)"
            )
            sys.stdout.flush()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=cpu_count) as executor:
        for idx, result in executor.map(
            is_very_silly_with_index, enumerate(all_numbers)
        ):
...
with open("02.txt") as f:
    all_numbers = [
        n
        for str_range in f.readlines()[0].strip().split(",")
        for n in iterate_range(str_range)
    ]
    ...
    answer2 = sum(
        n for n, very_silly in zip(all_numbers, is_very_silly_results) if very_silly
    )
